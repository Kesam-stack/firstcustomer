...
import { ConversionRiskActions, PayoutRetry } from "@/components/AdminActions";
  ...
  return <main className="shell page">
    ...
    <div className="table-card">
      <div className="table-scroll"><table>
        <thead><tr><th>When</th><th>Company</th><th>Referrer</th><th>Customer</th><th>Amount</th><th>Risk</th><th>Status</th><th></th></tr></thead>
        <tbody>{rows.length ? rows.map((row) => <tr key={row.id}>
          ...
          <td><span className="status-pill">{row.fraud_status} · {row.fraud_score}</span>{row.fraud_reasons?.length ? <div className="fineprint">{row.fraud_reasons.join(", ")}</div> : null}<ConversionRiskActions id={row.id} risk={row.fraud_status} /></td>
          ...
          <td><PayoutRetry id={row.id} status={row.payout_status} blocked={row.fraud_status !== "clear" || row.status !== "approved"} /></td>
        </tr>) : <tr><td colSpan={8}>No payouts.</td></tr>}</tbody>
      </table></div>
    </div>
  </main>;
}
