  ...
  try {
    ...
    const conversionId = await withTransaction(async (db) => {
      ...
      const inserted = await db.query<{ id: string }>(
        "INSERT INTO conversion_claims(bounty_id,referral_id,customer_reference,status,reward_cents,payout_status,approved_at) VALUES($1,$2,$3,'approved',$4,$5,NOW()) RETURNING id",
        [id, referralId, customerReference, lock.rows[0].reward_cents, payoutStatus],
      );
    ...
    const payout = bounty.payout_mode === "stripe" ? await attemptAutomaticPayout(conversionId) : { status: "manual_due" };
