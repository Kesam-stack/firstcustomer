...
export async function publishComplimentaryListing(input: ComplimentaryInput) {
  ...
  if (existing.rows[0]) {
    await query(
      `UPDATE bounties SET
       ...
       WHERE slug=$1`,
      [slug, companyName, productUrl, creatorEmail, creatorXHandle, companyDescription, category, logo, headline, desiredAction, referralTerms, rewardDollars * 100, goalCount, payoutMode],
    );
  ...
  const inserted = await query<{ id: string }>(
    `INSERT INTO bounties(
     ...
     RETURNING id`,
    [slug, hashToken(ownerKey), hashToken(integrationKey), integrationKey.slice(0, 12), creatorEmail, creatorXHandle, companyName, productUrl, companyDescription, category, logo, headline, desiredAction, referralTerms, rewardDollars * 100, goalCount, payoutMode, config.platformFeeBps],
  );
